...
if(isset($_GET['update']))
{
	...
	if( isset($_FILES['img']['name'])&& !empty($_FILES['img']['name']))
	{
		...
		$update="UPDATE `tbl_food` SET `title` = '$name',  `price` = '$price', `image_name` = '$dst_db', `category_id` = '$category', `featured` = '$featured', `active` = '$active', `quantity` = '$quantity' WHERE `tbl_food`.`id` = '$id'";
		...
		$update="UPDATE `tbl_food` SET `title` = '$name',  `price` = '$price', `category_id` = '$category', `featured` = '$featured', `active` = '$active', `quantity` = '$quantity' WHERE `tbl_food`.`id` = '$id'";
		if(mysqli_query($con,$update)){
			header("Location:m-prod.php");
		}else{
			header("Location:m-prod.php");
...
?>
	...
	<h1 class="page-header text-center" style="color:white;">Update Product</h1>
	<style>
		.form-control{
			width: 50%;
			margin-left: 25%;
